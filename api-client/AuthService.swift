...
class AuthService {
    ...
    func registerUser(username: String, password: String, completion: @escaping callback) {
        ...
        do {
            ...
            let task = session.dataTask(with: request, completionHandler: { (data: Data?, response: URLResponse?, error: Error?) -> Void in
                if error == nil {
                            ...
                            if result != nil {
                                if let email = result?["user"] as? String {
                                    if let token = result?["token"] as? String {
                                        //Successfully auth and have a token
                                        self.email = email
                    ...
                    //Failure
                    print("URL session task faield: \(error!.localizedDescription)")
                    completion(false)
                    return
                }
            })
